...
graphdb = neo4j.GraphDatabase( "neo4j-db/path" )
...
------------------------------------------------
 ...
 Indexes are created lazily. Unless you actually add some data to the index, 
 it will not be created on disk and not be abailable on later database startups.
...
* Traversals
 ...
 traversal class.) A traversal class needs to define the following members:
    ...
    [stop      ] Definition of when the traversal should stop.
                 Valid values are STOP_AT_END_OF_GRAPH and StopAtDepth(x)
